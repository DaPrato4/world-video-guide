...
export default function UserVideoList({
  ...
  const [rejectionOverlay, setRejectionOverlay] = useState<{
    title: string;
    reason: string;
  } | null>(null);
  ...
  return (
                      ...
                      <button
                        ...
                        onClick={() =>
                          setRejectionOverlay({
                            ...
                            reason: video.rejectionReason || "Motivazione non disponibile",
                          })
                        }
                        className="mt-2 inline-flex items-center gap-2 px-3 py-2 text-xs font-semibold text-red-300 bg-red-500/10 border border-red-500/20 rounded-lg hover:bg-red-500/20 transition-colors"
                      >
                        <FaExclamationCircle className="w-3.5 h-3.5" />
                        Vedi motivo del rifiuto
                      </button>
                    )}
    ...
    {rejectionOverlay && (
      <div
        ...
        onClick={() => setRejectionOverlay(null)}
      >
        <div
          className="w-full max-w-sm bg-neutral-900 border border-red-500/30 rounded-2xl shadow-2xl p-4 sm:p-5 animate-in fade-in zoom-in-95 duration-200"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="flex items-start justify-between gap-3 mb-3">
            <div className="flex items-center gap-2">
              <FaExclamationCircle className="text-red-400 w-4 h-4 shrink-0" />
          ...
          <p className="text-sm text-red-100/90 leading-relaxed italic bg-red-500/10 border border-red-500/20 rounded-xl p-3">
